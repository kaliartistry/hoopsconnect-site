...
- `CourtsideOperationServerAdapter` is the only network seam. It receives the immutable operation plus the preparation checksum, assignment/roster binding, and exact candidate identity. An accepted response must repeat that revision identity and package checksum around the operation receipt. The complete revision-bound envelope is stored with the accepted entry and independent receipt tombstone, retained in recovery archives, and revalidated during restart, exact replay, integrity scans, and pruning. A mismatched response or locally forged N/N+1 checkpoint cannot become accepted or be resent directly.
...
Focused tests cover loss before and after the local commit, exact state publication, lost server response and exact retry, restart with stranded sending state, duplicate/out-of-order receipts, concurrent capture serialization, second-device writer conflict, assignment revocation without unsafe retry, verified reauthentication, stale-state/new-command refusal, mismatched revision receipts, fully pruned accepted recovery, unavailable/quota-constrained/corrupt storage, sign-out preservation, deletion manifest/consent binding, hostile N/N+1 snapshot reuse, locally forged N+1 checkpoint evidence backed by an N receipt after pruning, mixed operator-resolution and reauthentication rows, closed-PWA copy, and keyboard focus safety. Existing journal, Flutter, analyze, dormancy, and repository-safety commands must remain green when this packet is integrated.
