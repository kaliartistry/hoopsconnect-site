...
| Minimized active-member directory | `buildActiveMemberDirectoryV2`; shared Dart parser | The builder rejects mixed project, tenant, or association inputs. Its DTO contains only schema, UID, display name, team ID, and division ID. Email, role, capabilities, FCM tokens, preferences, `G`, and `E` are absent. No production callable is exported. |
...
| FCM registration and detach | Candidate client adapter and candidate Rules | Each account has eight fixed installation slots, exact-scope/G/E bound and used as the sole notification delivery source. Token writes serialize. A stale completion is removed. Both sign-out and account switch acquire the client latch synchronously and require successful owner removal or token rotation before Auth may change. Failed Auth sign-out can restore the prior exact attempt only when registration restoration and two exact-current Auth checks succeed; otherwise grants stay latched closed. Public gate views are live and cannot cache a retired grant. Deletion cannot be rolled back. No production token service changes. |
