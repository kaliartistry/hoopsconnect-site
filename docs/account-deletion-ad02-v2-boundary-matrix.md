...
| FCM installation | `accountFcmRegistrationsV2Root/{uid}/installations/{installationId}` | `accountFcmRegistrationsV2Tenants/{tenantId}/users/{uid}/installations/{installationId}` |
...
| --- | --- | --- |
| Provider-bound authorization | `functions/src/domain/account_lifecycle_ad02_v2.ts` | Extracts only provider-verified V2 claims and calls the merged evaluator; creation time, V1 claims, profiles, and legacy memberships are never consulted. |
...
| Minimized active-member directory | `buildActiveMemberDirectoryV2`; shared Dart parser | DTO contains only schema, UID, display name, team ID, and division ID. Email, role, capabilities, FCM tokens, preferences, `G`, and `E` are absent. No production callable is exported. |
| Notification recipient and delivery ordering | `functions/src/domain/notification_delivery_ad02_v2.ts` | Tokens come only from current exact-scope/G/E installation registrations, never a parallel profile token list. Missing preferences are not opt-in. Effect and attempt fingerprints bind project/tenant/UID/G/E and token hashes. Each chunk is revalidated at reservation and dispatch. Provider invocation precedes the durable `submitted` claim. A dispatch barrier is released only with a durable terminal outcome. Existing and uncertain attempts are never resent. |
...
| Candidate routing | `lib/models/account_deletion/account_lifecycle_ad02_v2.dart` | Pure synthetic-state routing preserves `/delete-account` intent without reading raw lifecycle authority or changing `GoRouter`. No production route is installed. |
| FCM registration and detach | Candidate client adapter and candidate Rules | One installation record is exact-scope/G/E bound and is the notification delivery source. Token writes serialize. A stale completion is removed. Sign-out is blocked only if both owner removal and installation rotation fail. Failed Auth sign-out restores the previous exact attempt only after registration restoration succeeds; otherwise grants stay latched closed. Concurrent account-switch observation is serialized and deletion cannot be rolled back. No production token service changes. |
| Legacy derived writers | `evaluateLegacyDerivedWriterFenceV2`, `assertLegacyDerivedWriterFenceV2`, `commitCandidateDerivedWriteV2` | Source approval must carry original project/tenant/UID/G/E. Every candidate derived commit rechecks that exact stamp inside its transaction. A delayed trigger cannot attach `approvedBy` UID to whatever authority is current later. Production legacy writers are unchanged. |
| Firestore and Storage | Emulator-only AD02 fixtures | Firestore uses token plus lifecycle/membership and stamps protected records with full project/tenant/UID/G/E ownership. Storage uses one exact V2 projection plus root/tenant/UID/G-separated physical object lanes. Raw authority/deletion/delivery records are private. Generic Storage mutation remains denied. `firebase.json` still references only live V1 Rules. |
